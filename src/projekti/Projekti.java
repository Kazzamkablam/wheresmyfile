...
final class varDefinitions {
	...
	public static String[] listChoices = { "Everything", "My Images", "My Documents", "My Videos" }; // choices for drop
																										// down, if I
																										// wanted to go
																										// fancy I could
																										// make these
...
class SQLQueries { // sql queries class
	...
	private static final String URL = "jdbc:mysql://penttinen-juha.mysql.database.azure.com:3306/myfiles?useSSL=true&requireSSL=false&useLegacyDatetimeCode=false&serverTimezone=UTC"; // need
																																														// timezone
																																														// stuff
																																														// to
																																														// get
	...
	public static boolean OpenConnection() { //open connection to database
		try {
	...
	public static boolean getUserData(int UserID) { //get user data

		try {
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery(
					"SELECT FileID, FileName, FileDesc, FileType, UserID FROM files WHERE UserID ='" + UserID + "'");
			while (rs.next()) {
				...
				// public static boolean addFile(int gID, String name, String path, int index) {
				// //add an entry
			}
			return true;
		} catch (SQLException err) {
	...
	public static int getUser(String Username, String Password) { // get user ,return false if wrong
		...
		try {
			Statement stmt = conn.createStatement();
			ResultSet rs = stmt.executeQuery("SELECT UserID, UserName, PassWord FROM users WHERE userName='" + Username
	...
	public static boolean createUser(String Username, String Password) { // create new user
		try {
			...
			PreparedStatement ps = conn
					.prepareStatement("INSERT INTO users SET UserID = ?, UserName = ?, PassWord = ?"); //prepared statement
			ps.setInt(1, 0);
	...
	public static int createEntry(int UserID, String filename, String filedesc, int filetype) { // create new entry
		try {
			...
			PreparedStatement ps = conn.prepareStatement( //prepared statement
					"INSERT INTO files SET FileID = ?, FileName = ?, FileDesc = ?, FileType = ?, UserID = ?",
	...
	public static boolean updateEntry(int FileID, int UserID, String filename, String filedesc, int filetype) { // update
																												// entry
		try {
	...
	public static boolean deleteEntry(int FileID) { // delete entry
		try {
			...
			PreparedStatement ps = conn.prepareStatement("DELETE FROM files WHERE FileID = ?"); //prepared statement
			ps.setInt(1, FileID);
			ps.executeUpdate();
...
class Projekti { // main class of the project, handles functionality of main UI, probably should change name of this, but Eclipse not allowing me to without hassle, so leaving as it is.
	...
	private static void refreshList(JList<Object> listItem) { // refresh contents of list item
		...
		if (itemListing.getSelectedIndex() == varDefinitions.itemEverything) {
			...
			for (int i = 0; i < fileDataBase.getSize(); i++) {
				if (fileDataBase.getIndexItem(i) == itemListing.getSelectedIndex()) { // only display selected item
																						// category.
	...
	private static int cIndex() { // convert index to get proper file id.
		...
		if (itemListing.getSelectedIndex() == varDefinitions.itemEverything) {
			...
			convertIndex = referenceIndex.get(fileNames.getSelectedIndex()); // convert to right index to get right
																				// file.
		}
		return convertIndex;
	};
	...
	private static void createAndShowGUI() { // Main UI
		...
		btnNewFile.addActionListener(new ActionListener() { // new menu listener
		...
		itemListing.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent arg0) { // state change listener, should this happen refresh contents
															// of listbox
		...
		btnUpdateFile.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) { // update file
				if (fileNames.getSelectedIndex() != -1)
					displayFileAdd(1);
			}
		});
		...
		JButton btnExit = new JButton("Exit"); // exit button functionality
		...
		JPanel bottomContent = new JPanel(); // panels for making main UI
		...
		fileNames.setSelectionMode(ListSelectionModel.SINGLE_SELECTION); // allow only single item selection

		fileNames.addListSelectionListener(new ListSelectionListener() { // listener for selection changes
			public void valueChanged(ListSelectionEvent arg0) {
				if (fileDataBase.getSize() > fileNames.getSelectedIndex() && fileNames.getSelectedIndex() != -1)
					lblFileInformation.setText(fileDataBase.getIndexPath(fileNames.getSelectedIndex())); // index is not
																											// out of
																											// bounds
		...
		fileNames.addMouseListener(new MouseAdapter() { // add listener for double clicks
			@Override
	...
	private static boolean displayLogin() { // code for login function and box layout
		...
		if (result == 0) { // chose log in
			SQLQueries.OpenConnection(); // open connection to database
			...
			if (connected != -1) { // connection successful
	...
	private static void displayFileAdd(int type) { // display file dialog box with option for update or new file.
		...
		if (type == 0) { // regular new file
			int result = JOptionPane.showConfirmDialog(null, menuPanel, "Enter details for file to add",
					JOptionPane.OK_CANCEL_OPTION);

			if (result == JOptionPane.OK_OPTION) {
				int generatedID = SQLQueries.createEntry(SQLQueries.getConnectedUser(), itemLocation.getText(),
						itemDescription.getText(), itemCategory.getSelectedIndex()); // create entry to database
				fileDataBase.addFile(generatedID, itemLocation.getText(), itemDescription.getText(),
						itemCategory.getSelectedIndex()); // reference in our list too
				refreshList(fileNames); // refresh the list
			}
		...
		if (type == 1) { // modify file
			...
			int result = JOptionPane.showConfirmDialog(null, menuPanel, "Enter details for file to modify",
					JOptionPane.OK_CANCEL_OPTION);

			if (result == JOptionPane.OK_OPTION) {

				try {
					// public static int UpdateEntry(int FileID, int UserID, String filename, String
					// filedesc, int filetype) { //update entry
					SQLQueries.updateEntry(itemID, SQLQueries.getConnectedUser(), itemLocation.getText(),
							itemDescription.getText(), itemCategory.getSelectedIndex());
					fileDataBase.updateFile(fileDataBase.getItemID(convertIndex), convertIndex,
							itemDescription.getText(), itemLocation.getText(), itemCategory.getSelectedIndex());
					// fileDataBase.addFile( itemLocation.getText(), itemDescription.getText(),
					// itemCategory.getSelectedIndex());
					refreshList(fileNames); // refresh the list
